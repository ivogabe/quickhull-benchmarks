...
initialize points = T2 newPoints hull
  where
    ...
    line@(T2 leftMost rightMost)
      = the $ fold
        (\(T2 min1 max1) (T2 min2 max2) -> T2 (min min1 min2) (max max1 max2))
...
initializeSplit points = T2
  ...
  where
    ...
    hull1 :: Acc (Vector Point)
    hull1 = generate (Z_ ::. 2) (\(I1 idx) -> idx == 0 ? (leftMost, rightMost))

    hull2 :: Acc (Vector Point)
    hull2 = generate (Z_ ::. 2) (\(I1 idx) -> idx == 0 ? (rightMost, leftMost))
...
stepFlat (T2 pointsWithLabels hull) = T2 newPoints newHull
  where
    ...
    newPoints :: Acc (Vector (Point, Label))
    newPoints
      = permute const (generate (I1 $ the newSize) $ const Unsafe.undef) destination
...
stepToFlat (T2 points line') = T2 newPoints newHull
  where
    ...
    destination :: Exp DIM1 -> Exp (Maybe DIM1)
    destination idx
      = cond (g == 0) (Just_ $ I1 $ leftOffset)
      $ cond (g == 1) (Just_ $ I1 $ leftSize + rightOffset)
      Nothing_
      where
        g = newLabels ! idx
    ...
    newPoints :: Acc (Vector (Point, Label))
    newPoints
      = permute const (generate (I1 $ leftSize + rightSize) $ const Unsafe.undef) destination $ zip points newLabels
